...
ArbolBinarioBusqueda abb_crear() {
  ArbolBinarioBusqueda nuevo_arbol =
      (ArbolBinarioBusqueda)malloc(sizeof(struct ArbolBinarioBusquedaRep));
  nuevo_arbol->raiz = NULL;
  nuevo_arbol->cantidad_elementos = 0;
  return nuevo_arbol;
}

bool abb_es_vacio(ArbolBinarioBusqueda a) { return (a->raiz == NULL); }
...
bool abb_es_rama_nula(NodoArbol pa) { return pa == NULL; }

NodoArbol abb_raiz(ArbolBinarioBusqueda a) { return a->raiz; }

NodoArbol abb_insertar_recursivo(ArbolBinarioBusqueda a, NodoArbol pa,
                                 TipoElemento te, bool *inserto) {
  if (pa == NULL) {
    *inserto = true;
...
bool abb_eliminar(ArbolBinarioBusqueda a, int claveABorrar) {
  ...
  // cotemplo que si borra la raiz y no tiene hijos por la derecha el hijo
  // izquierdo se convierte en raiz
  ...
  // cotemplo que si borra la raiz y no tiene hijos por la izquierda el hijo
  // derecho se convierte en raiz
