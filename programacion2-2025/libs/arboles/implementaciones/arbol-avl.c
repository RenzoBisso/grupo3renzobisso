...
bool avl_es_vacio(ArbolAVL a) { return a->raiz == NULL; }

bool avl_es_lleno(ArbolAVL a) {
  return (a->cantidad_elementos == TAMANIO_MAXIMO);
}

bool avl_es_rama_nula(NodoArbol pa) { return pa == NULL; }

int avl_cantidad_elementos(ArbolAVL a) { return a->cantidad_elementos; }

NodoArbol avl_raiz(ArbolAVL a) { return a->raiz; }
...
NodoArbol avl_eliminar_recursivo(ArbolAVL arbol, NodoArbol nodoArbol,
                                 int claveABorrar, bool *borre) {
  if (nodoArbol == NULL) {
    arbol->cantidad_elementos++;  // No lo encontramos, sumamos para que quede
                                  // igual que antes
    return nodoArbol;
  }
  ...
  if (claveABorrar < nodoArbol->datos->clave)
    nodoArbol->hi =
        avl_eliminar_recursivo(arbol, nodoArbol->hi, claveABorrar, borre);
  else if (claveABorrar > nodoArbol->datos->clave)
    nodoArbol->hd =
        avl_eliminar_recursivo(arbol, nodoArbol->hd, claveABorrar, borre);
  else {
    // encontre la clave
    *borre = true;
    // es una hoja
    if (nodoArbol->hi == NULL && nodoArbol->hd == NULL) {
      ...
      // El nodo tiene 2 hijos, buscamos el sucesor en in-orden (o sea el menor
      // del subárbol de la derecha)
      ...
      nodoArbol->hd = avl_eliminar_recursivo(arbol, nodoArbol->hd,
                                             temp->datos->clave, borre);
    }
  }
  ...
  if (balanceState == DESBALANCEADO_DERECHA) {
    ...
    // avl_calcular_balanceo(nodoArbol->hd) ===
    // Balanceo.APENAS_DESBALANCEADO_DERECHA
