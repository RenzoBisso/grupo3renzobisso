...
int leer_entero_no_negativo() {
    ...
    while (1) {
        ...
        if (sscanf(buffer, "%d %c", &valor, &extra) == 1) {
            ...
            printf(
                "Entrada inválida. Por favor ingrese un número entero >= 0.\n");
        }
    }
}
...
void crearBinarioAleatorio(ArbolBinario b, NodoArbol avl, NodoArbol raiz) {
    if (!a_es_rama_nula(avl)) {
        if (n_hijoderecho(avl) != NULL) {
            NodoArbol x =
                a_conectar_hd(b, raiz, n_recuperar(n_hijoderecho(avl)));
            crearBinarioAleatorio(b, n_hijoderecho(avl), x);
        }
        if (n_hijoizquierdo(avl) != NULL) {
            NodoArbol y =
                a_conectar_hi(b, raiz, n_recuperar(n_hijoizquierdo(avl)));
...
void lRandomSinRepetir(Lista l, int min, int max, int cantidad, int count) {
    if (cantidad > (max - min + 1)) {
        printf("Error: no hay suficientes números únicos en el rango [%d, %d] "
               "para generar %d valores sin repetir.\n",
               min, max, cantidad);
        return;
    }
    ...
    TipoElemento te = te_crear(numero_aleatorio(min, max));

    if (l_buscar(l, te->clave) != NULL) {
        free(te);
        return lRandomSinRepetir(l, min, max, cantidad, count);
    }

    l_agregar(l, te);
    lRandomSinRepetir(l, min, max, cantidad, count + 1);
}
// FUNCIONES DE ARBOLES
// ####################
// ####################